...
INDICATORS: dict[str, dict] = {
    ...
    "exports": {
        ...
        "date_col": "日期",
        "value_cols": {"今值": "export_yoy"},
        "desc": "出口年率（美元计，外需维度；与 CPI 同为报告日期型数据源）",
    },
    "market": {
        # 无标准 (date_col, value_cols) 形态：日频指数需聚合成月频，走自定义 loader
        "custom": "load_hs300_monthly",
        "desc": "沪深300 月收益率 + 月内已实现波动（市场情绪，月末即时可得，零发布滞后）",
    },
}
...
PUBLICATION_LAG: dict[str, int] = {
    ...
    "export_yoy": 1,   # 海关出口数据次月上旬（与 CPI 同为报告日期型，保守取 1）
    "hs300_ret": 0,    # 指数价格月末收盘即知，唯一零滞后的非 PMI 特征
    "hs300_vol": 0,
}
